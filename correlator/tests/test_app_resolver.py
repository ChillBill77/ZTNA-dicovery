...
async def test_manual_match_wins(redis) -> None:
    ...
    cand = await r.resolve(
        dst_ip="10.100.0.5", dst_port=443, proto=6, firewall_fqdn=None, app_id=None
    )
...
async def test_firewall_fqdn_matches_saas(redis) -> None:
    ...
    cand = await r.resolve(
        ...
        dst_port=443,
        proto=6,
        firewall_fqdn="outlook.office365.com",
        app_id=None,
    )
...
async def test_ptr_fallback_via_redis(redis) -> None:
    ...
    cand = await r.resolve(dst_ip="8.8.8.8", dst_port=53, proto=17, firewall_fqdn=None, app_id=None)
...
async def test_port_default_when_no_fqdn(redis) -> None:
    ...
    cand = await r.resolve(
        dst_ip="10.0.0.99", dst_port=22, proto=6, firewall_fqdn=None, app_id=None
    )
...
async def test_raw_fallback(redis) -> None:
    ...
    cand = await r.resolve(
        dst_ip="198.51.100.1", dst_port=9999, proto=6, firewall_fqdn=None, app_id=None
    )
...
async def test_missing_ptr_enqueues_for_resolver(redis) -> None:
    r = AppResolver(redis=redis)
    r.load(manual=[], saas=[], port_defaults=[])
    await r.resolve(dst_ip="203.0.113.5", dst_port=443, proto=6, firewall_fqdn=None, app_id=None)
...
async def test_reload_replaces_caches(redis) -> None:
    ...
    cand = await r.resolve(
        dst_ip="10.0.0.1", dst_port=443, proto=6, firewall_fqdn=None, app_id=None
    )
