...
def test_load_adapter_configs(tmp_path: Path) -> None:
    ...
    assert configs["palo_alto"] == AdapterConfig(
        enabled=True, source_ips=frozenset({"10.0.0.1"})
    )
    assert configs["fortigate"] == AdapterConfig(
        enabled=False, source_ips=frozenset()
    )
